...
/* Ranking signals — surfaced as the clickable "Sorted by" chips.
   Toggling a signal off re-weights and re-ranks the priority modules live. */
const SORT_SIGNALS = [
  { key: "usage",   label: "7-day usage",         weight: 0.40 },
  { key: "recency", label: "recent activity",     weight: 0.25 },
  { key: "urgency", label: "operational urgency", weight: 0.20 },
  { key: "role",    label: "role relevance",      weight: 0.15 }
];
...
/* Rank modules using only the active signals; weights renormalise so the
   displayed score always reads 0–100 regardless of which chips are on. */
function rankModules(activeSignals) {
  const maxVisits = Math.max(...MODULES.map(m => m.usage.visits7d), 1);
  const now = Date.now();
  const active = SORT_SIGNALS.filter(s => activeSignals[s.key]);
  const totalW = active.reduce((a, s) => a + s.weight, 0) || 1;
  ...
  return MODULES
    .map(m => {
      ...
      let score = 0;
      for (const s of active) score += (s.weight / totalW) * parts[s.key];
      return { ...m, _score: score, displayScore: Math.round(score * 100) };
    })
    .sort((a, b) => b._score - a._score)
    .map((m, i) => ({ ...m, rank: i + 1 }));
}

/* Rule 1: pin the top 4 highest-scoring modules */
function getPriorityModules(activeSignals) { return rankModules(activeSignals).slice(0, 4); }
